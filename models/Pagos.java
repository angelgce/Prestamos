...
public class Pagos {
    ...
    public void pagosPendientes(String cliente) {
        ...
        Main.prestamosList.stream().filter(user -> user.getCliente().equals(cliente))
                ...
                .forEach(item -> System.out.println("Deuda total: $" + calcularPago(item) + "\n"));

        // validamos si hubo datos
        long count = Main.prestamosList.stream().filter(user -> user.getCliente().equals(cliente))
                .filter(status -> status.getEstado().equalsIgnoreCase("pendiente")).count();
        if (count == 0) {
